...
The diagrams show the algorithmic complexity being evaluated. Runtime measurements are generated by `benchmark_extended()` in `booking_system.py`.
...
Run the DSA unit tests with:

```bash
python -m unittest discover -s tests -v
```

The test suite covers the custom hash table, binary min-heap, 2-D seat grid, search algorithms, and sorting algorithms.
